...
(function (ts) {
    ts.Diagnostics = {
        ...
        A_0_parameter_must_be_the_first_parameter: { code: 2680, category: ts.DiagnosticCategory.Error, key: "A_0_parameter_must_be_the_first_parameter_2680", message: "A '{0}' parameter must be the first parameter." },
        ...
        JSDoc_types_can_only_be_used_inside_documentation_comments: { code: 8020, category: ts.DiagnosticCategory.Error, key: "JSDoc_types_can_only_be_used_inside_documentation_comments_8020", message: "JSDoc types can only be used inside documentation comments." },
...
(function (ts) {
    ...
    function isJSDocConstructSignature(node) {
        return node.kind === 273 &&
            ...
            node.parameters[0].name.text === "new";
    }
    ...
    function getJSDocParameterTags(param) {
        if (param.name && ts.isIdentifier(param.name)) {
            ...
            return getJSDocTags(param.parent).filter(function (tag) { return ts.isJSDocParameterTag(tag) && tag.name.text === name_1; });
        }
        else {
            return undefined;
        }
        ...
        (function (JSDocParser) {
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function parseTypedefTag(atToken, tagName) {
                    ...
                    function isObjectTypeReference(node) {
                        return node.kind === 134 ||
                            ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) && node.typeName.text === "Object";
                    }
...
(function (ts) {
    ...
    (function (JsTyping) {
        ...
        function discoverTypings(host, log, fileNames, projectRootPath, safeListPath, packageNameToTypingLocation, typeAcquisition, unresolvedImports) {
            ...
            if (!safeList) {
                var result_2 = ts.readConfigFile(safeListPath, function (path) { return host.readFile(path); });
                safeList = ts.createMapFromTemplate(result_2.config);
            }
            var filesToWatch = [];
            ...
            if (unresolvedImports) {
                var x = unresolvedImports.map(function (moduleId) { return nodeCoreModules.has(moduleId) ? "node" : moduleId; });
                if (x.length && log)
                    log("Inferred typings from unresolved imports: " + JSON.stringify(x));
                for (var _a = 0, x_1 = x; _a < x_1.length; _a++) {
                    var typingName = x_1[_a];
            ...
            function getTypingNamesFromJson(jsonPath, filesToWatch) {
                ...
                if (log)
                    log("Searching for typing names in '" + jsonPath + "' dependencies");
            ...
            function getTypingNamesFromSourceFileNames(fileNames) {
                ...
                if (fromFileNames.length) {
                    if (log)
                        log("Inferred typings from file names: " + JSON.stringify(fromFileNames));
                    for (var _i = 0, fromFileNames_1 = fromFileNames; _i < fromFileNames_1.length; _i++) {
                        var safe = fromFileNames_1[_i];
            ...
            function getTypingNamesFromPackagesFolder(packagesFolderPath, filesToWatch) {
                ...
                for (var _i = 0, fileNames_3 = fileNames; _i < fileNames_3.length; _i++) {
                    ...
                    var result_3 = ts.readConfigFile(normalizedFileName, function (path) { return host.readFile(path); });
                    var packageJson = result_3.config;
        ...
        (function (typingsInstaller) {
            ...
            var TypingsInstaller = (function () {
                ...
                TypingsInstaller.prototype.install = function (req) {
                    ...
                    var discoverTypingsResult = ts.JsTyping.discoverTypings(this.installTypingHost, this.log.isEnabled() ? this.log.writeLine : undefined, req.fileNames, req.projectRootPath, this.safeListPath, this.packageNameToTypingLocation, req.typeAcquisition, req.unresolvedImports);
                    if (this.log.isEnabled()) {
